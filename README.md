...
Tailored for high-core systems, multi-socket workstations, and Intel Xeon E5 v3/v4 (Hack TBU) platforms where dynamic workload switching between high-frequency gaming, heavy multi-threading, and ultra-low power idle is required without touching BIOS settings.
...
* **Maximize Turbo Headroom:** Offlining 6–8 cores leaves the full TDP limit (e.g., 145W PL1) available to the active cores, sustaining full all-core Turbo Boost (up to 3.80 GHz) without power-throttling.
* **Instant Hyperthreading Toggle:** Enable or disable SMT on the fly with a single command.

### 2. Tailored Governor & Frequency Profiles
* **`fast`:** All online cores on `schedutil` with full Turbo Boost enabled (up to 3.8 GHz).
* **`base` (or `noboost`):** All online cores on `schedutil` clamped strictly to base frequency (1.2 GHz – 2.3 GHz, 0% Turbo). Ideal for cool, quiet multi-threaded crunching at ~70W.
* **`eco`:** All online cores locked to `powersave` at 1.2 GHz (~24W package idle floor).
* **`split`:** Automated asymmetric cluster. Keeps primary cores fast at 3.8 GHz while locking secondary cores at 1.2 GHz powersave for background tasks or AFK games.
...
# Bring all 18 physical cores back online
...
```bash
# Full Performance mode (Schedutil up to 3.8 GHz Turbo)
cpu-profile fast

# Base / No-Boost mode (Schedutil capped @ 2.3 GHz, 0% Turbo, quiet & cool)
cpu-profile base

# Full Eco mode (All cores locked @ 1.2 GHz, minimum power draw)
cpu-profile eco
...
* **Frequency Caps:** Modifies `/sys/devices/system/cpu/cpu*/cpufreq/scaling_max_freq` and `intel_pstate/no_turbo` to control ceilings without introducing voltage instability.
